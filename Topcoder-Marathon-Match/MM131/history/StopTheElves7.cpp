...
bool can_build(int n, int b) {
    return (numElf + turnLeft * elfP) < (numPresent - n) + (boxLeft - b);
}
...
int set_wall() {
    ...
    while (true) {
        ...
        //cerr << touch[SZ(touch) - 1] / N << " " << touch[SZ(touch) - 1] % N << endl;
...
void solve(int turn) {
    ...
    REP(i, N * N)if (draft[i] == 'a') {
        nb++;
    }
    if (nb <= money / C) {
        REP(p, N* N)if (draft[p] == 'a'&&grid[p].state=='.') {
            install.emplace_back(p);
            grid[p].state = 'b';
            draft[p] = 'b';
            boxLeft--;
