...
int set_wall() {
    ...
    while (true) {
        ...
        //if (SZ(touch) < prevNP+5&&elapsedTime < TIME_LIMIT)update_wall(nb);
        if (numPresent+numCareer < prevNP + 5 && elapsedTime < TIME_LIMIT)update_wall(nb);
...
void solve(int turn) {
    ...
    set_around();
    int np=0;
    if ((prevP != numPresent || numCareer>0)) {
        np=set_wall();
        prevP = numPresent;
        prevE = numCareer;
    }
