...
double elfP;
V<bool> border;
V<int> elves,careers,presents,install;
    ...
    REP(x, N)REP(y, N) {
        ...
        REP(i, 4) {
            ...
            if (nx < 0 || nx >= N || ny < 0 || ny >= N
                || grid[pos(nx, ny)].state == 'T' || grid[pos(nx, ny)].state == 'b')continue;
            grid[pos(x, y)].around4.emplace_back(pos(nx, ny));
        }
        shuffle(ALL(grid[pos(x, y)].around4), rng);
    }
...
void init() {
    ...
    REP(i, N)REP(j, N) {
        ...
        if (i == 0 || i == N - 1 || j == 0 || j == N - 1)border[pos(i, j)] = true;
    }
    cerr << "treeP : " << double(numTree)/(N*N) << endl;
    cerr << "presentP : " << double(numPresent)/(N*N)<< endl;
