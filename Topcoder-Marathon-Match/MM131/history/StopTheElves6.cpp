...
int maxBox, numBox,maxPresent,turnLeft;
double elfP;
V<bool> border;
...
void solve2(int turn,bool &flag) {
    //if (numElve + turnLeft*elfP > (money + turnLeft) / C + numPresent)return;
    //if (true)return;
    if (numElve + numCareer == 0)return;
    set_reach();
    set_route();
    sort(careers.begin(), careers.begin() + numCareer, compE);
    sort(presents.begin(), presents.begin() + numPresent, compP);
...
void solve(int turn) {
    ...
    if (money / C == 0)return;
    if (money / C >= 4) {
        for (int r : grid[presents[0]].around4) {
            if (grid[r].state == '.') {
                install.push_back(r);
                grid[r].state = 'b';
                numBox++;
...
signed main(int argc, char* argv[]) {
    ...
    if (N * N * elfP > numPresent + maxBox - ((low+99) / 100) * 5)quit = true;
    REP(turn,N*N){
        install = V<int>();
        turnLeft = N * N - turn - 1;
