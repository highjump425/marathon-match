...
const ll T0 = 5, T1 = 1;
int N, C,pre_score,score,best_score,cnt=0,update_cnt,new_point,point;
VV<int> grid,best_goal,goal,temp,check,d;
V<int> color_map;
V<P> position,temp_pos;
V<P> color;
V<tuple<int,int,int,int>> best_ans,ans,best_route,route;
map<P, P> destination;
V<ll> y_vec = { 0, 1,0, -1 };
V<ll> x_vec = { -1, 0,1, 0 };
...
int get_score() {
    REP(i, N)REP(j, N) {
        if (temp[i][j] != grid[i][j])temp[i][j] = grid[i][j];
    }
    REP(i, N* N) {
        if (temp_pos[i] != position[i])temp_pos[i] = position[i];
    }
    int res = 0;
...
bool check_connect() {
    REP(i, N)REP(j, N)if (check[i][j])check[i][j] = 0;
    int num = 0;
    REP(i, N)REP(j, N) {
        if (!check[i][j]) {
            check_dfs(i, j);
...
void change_place() {
    ...
    if (check_connect()) {
        score = get_score();
        double prob = exp((score - pre_score) / get_temp());
        if (prob > dist(rng)) {
            if (chmin(best_score, score)) {
                best_goal = goal;
                update_cnt++;
...
void move_dfs(int x, int y,int tx,int ty) {
    if (x == tx && y == ty) {
        if (chmax(new_point, point)) {
            best_route = route;
        }
        return;
    }
    if (tx > x&& d[tx - 1][ty] == 0) {
        if (temp_pos[best_goal[tx - 1][ty]].first > tx - 1)point++;
        else point--;
        route.emplace_back(make_tuple(tx, ty, tx - 1, ty));
        swap(temp_pos[temp[tx][ty]], temp_pos[temp[tx - 1][ty]]);
        swap(temp[tx][ty], temp[tx - 1][ty]);
        tx--;
        move_dfs(x, y, tx, ty);
    ...
    if (ty > y) {
        if (temp_pos[best_goal[tx][ty-1]].second > ty - 1)point++;
        else point--;
        route.emplace_back(make_tuple(tx, ty, tx, ty - 1));
        swap(temp_pos[temp[tx][ty]], temp_pos[temp[tx][ty - 1]]);
        swap(temp[tx][ty], temp[tx][ty - 1]);
        ty--;
        move_dfs(x, y, tx, ty);
...
void new_output() {
    ...
    REP(x, N)REP(y, N) {
        ...
        route= V<tuple<int, int, int, int>>();
        best_route = V<tuple<int, int, int, int>>();
        point = 0;
        new_point = -INF;
        move_dfs(x, y, tx, ty);
        REP(i, SZ(best_route)) {
            int x1, y1, x2, y2; tie(x1, y1, x2, y2) = best_route[i];
            ans.emplace_back(make_tuple(x1, y1, x2, y2));
            swap(temp_pos[temp[x1][y1]], temp_pos[temp[x2][y2]]);
