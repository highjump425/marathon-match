...
void greedy3() {
    REP(i, N) {
        temp_nodes[i].second = i;
        temp_nodes[i].first.second = -SZ(graph[i]);
    }
    sort(ALL(temp_nodes));
    REP(p, CINF) {
        bool flag = false;
        REP(i, N) {
            int x = temp_nodes[i].second;
            if (nodes[x] != -1)break;
            bool flag2 = true;
            for (int j : graph[x]) {
                if (nodes[j] == -1)continue;
        ...
        if (!flag)continue;
        REP(i, N)temp_nodes[i].first.first = edge_num[temp_nodes[i].second];
        //cnt++;
        //cerr << "turn: " << cnt << endl;
        sort(ALL(temp_nodes));
        if (temp_nodes[0].first.first >= N)break;
    }
}
...
signed main() {
    ...
    while (t_diff < TIME_LIMIT) {
        if (greedy2()&&chmin(best_ans, *max_element(ALL(nodes)))) {
            best_nodes = nodes;
        }
