...
public:
    ...
    bool operator<(const state& l) const{
        if (mode == 0) {
            ...
            if (this->now < N && l.now < N) {
                if (SZ(graph[this->id]) - this->now != SZ(graph[l.id]) - l.now) {
                    return SZ(graph[this->id]) - this->now > SZ(graph[l.id]) - l.now;
                }
                else {
                    if (SZ(graph[this->id]) != SZ(graph[l.id]))return SZ(graph[this->id]) < SZ(graph[l.id]);
                    else return this->id < l.id;
...
void greedy3() {
    ...
    REP(p, CINF) {
        ...
        bool flag = false;
        //int st = 0;
        //if(N>50&&turn<N/2)st = (dist_int(rng) % 3);
        FOR(i, 0,min(max(50,N/5),N)-1) {
        //REP(i,min(100,N)){
            int x = states[i].id;
            if (nodes[x] != -1)break;
            bool flag2 = true;
...
void one_change() {
    ...
    for (int x : graph[a]) {
        check_edge[abs(nodes[a] - nodes[x])] = false;
    }
    bool flag = false;
    if (nodes[a] >= best_ans/2) {
        FOR(p, 0, best_ans) {
            if (check_node[p])continue;
            bool flag = true;
            for (int x : graph[a]) {
                if (check_edge[abs(p - nodes[x])]) {
                    flag = false;
                    break;
                }
            }
            if (!flag)continue;
            for (int x : graph[a]) {
                check_edge[abs(p - nodes[x])] = true;
            }
            check_node[p] = true;
            nodes[a] = p;
            if(value!=p)flag=true;
            break;
        }
    }
    else {
    ...
    if (!flag)return;
    int max_node = max_element(ALL(nodes)) - nodes.begin();
    value = nodes[max_node];
    check_node[value] = false;
    for (int x : graph[max_node]) {
        check_edge[abs(nodes[max_node] - nodes[x])] = false;
    }
    FOR(p, 0, best_ans) {
        if (check_node[p])continue;
        bool flag = true;
    ...
    int min_node = min_element(ALL(nodes)) - nodes.begin();
    value = nodes[min_node];
    check_node[value] = false;
    for (int x : graph[min_node]) {
        check_edge[abs(nodes[min_node] - nodes[x])] = false;
    }
    FORD(p,best_ans,0) {
        if (check_node[p])continue;
        bool flag = true;
        ...
        if (value != p) {
            cerr << value << " " << p << endl;
            int min_value = *min_element(ALL(nodes));
            REP(i, N)nodes[i] -= min_value;
            if (chmin(best_ans, *max_element(ALL(nodes)))) {
                REP(i, N)if (best_nodes[i] != nodes[i])best_nodes[i] = nodes[i];
            }
        }
...
signed main() {
    ...
    nodes = best_nodes;
    REP(i, N)check_node[nodes[i]] = true;
    REP(i, E)check_edge[abs(nodes[edges[i].first] - nodes[edges[i].second])] = true;
