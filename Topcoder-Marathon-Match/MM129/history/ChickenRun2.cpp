...
void set_destination(int r) {
    ...
    if (r<N) {
        ...
        REP(i, N) {
            if (line[i] == true)continue;
            if ((i==N||line[i+1]==true) && (i>0&&line[i - 1] == false))continue;
            destinations[cnt] = MP(r, i);
            cnt++;
            line[i] = true;
            //if (i < N - 1)line[i + 1] = true;
...
void align() {
    REP(f, SZ(farmers)) {
        ...
        if (destinations[f].first == -1) {
            ...
            P dir = get_direction(p, destinations[f]);
            if (dir.first == -1 || grid[dir.first][dir.second] >= 2) {
                dir = get_direction2(p, destinations[f]);
